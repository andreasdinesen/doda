...
| Version | Ændringer |
|---|---|
| 56 | **Totrinsbekræftelse (2FA).** En sekscifret kode fra din authenticator, oven på kodeordet. Passkeys er stærkere, hvor de virker — men de kræver https, og panelet nås over almindelig http; **det hul lukker det her.** Opsætningen slår det først til, når du har tastet en kode, der passer, så du ikke kan låse dig ude ved at lukke fanen. Du får **ti nødudgangskoder**, vist én gang: hver virker én gang i stedet for en kode fra appen. Den samme kode kan **ikke** bruges to gange — vinduet er 30 sekunder, og uden den spærre kunne en opsnappet kode genbruges. Og den kan kun slås fra mod dit kodeord, så en ulåst skærm ikke rækker. Algoritmen er prøvet mod RFC 6238's egne testvektorer. |
