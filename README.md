...
| Version | Ændringer |
|---|---|
| 100 | **Tallet på app-ikonet.** Har du doda på hjemmeskærmen, viser ikonet nu, hvor mange opgaver der ligger i **Next Actions** — som Todoist gør. Tallet tælles som listen, så udskudte opgaver først tæller med på deres dag. Det opdateres, når du bruger doda, og mens doda er lukket følger det med hver påmindelse. **På iPhone kræver det, at notifikationer er slået til** for doda; ændrer du opgaver et andet sted, retter ikonet sig først, næste gang du åbner doda eller får en påmindelse — mere lader iOS ikke en webapp gøre. Kan slås fra under Settings → Notifications. |
...
| 94 | **En gentagelse, du afslutter via status, fortsætter nu.** Satte du en `!every!`-opgave til *Done* med status-knappen i opgaven — i stedet for ringen — blev den næste forekomst **aldrig** lavet: gentagelsen stoppede stille, og Recurring-siden blev ved med at vise den gamle dato. Det samme gjaldt, når Claude eller en genvej afsluttede den. Nu går alle veje det samme sted hen, og **»fra fuldførelse« regner fra den dag, du blev færdig**: sat til 20-9, afsluttet 21-9, næste gang 21-10. *Dropped* via status tæller som et spring og fortsætter også. |
| 93 | **Se hvor gammel en opgave er.** Åbn en opgave, og nederst står der fx *Created 3 weeks ago · 21 Aug 2026* — hold musen over for det præcise tidspunkt. **Det gælder også alle dine gamle opgaver**, med deres rigtige dato: doda har gemt, hvornår hver opgave blev oprettet, hele tiden. Den blev bare aldrig vist. Det er kun en oplysning — den kan ikke rettes, og den står ikke i listen. |
| 92 | **Et link i en titel vises som tekst — ikke som markdown.** Fanger du en opgave med et link i titlen, stod der `[Tjek om der ligger ordre](https://…)` i fuld længde. Nu står der **Tjek om der ligger ordre**, understreget og klikbart — i listen, på notekort, i gentagelser, i gennemgangen, i logbogen og i selve opgaven. **Vil du rette titlen**, klikker du bare på teksten ved siden af linket, så er markdown'en fremme igen. Og et klik på linket åbner nu kun linket — før åbnede det også opgaven bagved. |
| 91 | **Billedruden kom for sent — rettet.** I v90 åbnede et klik på et vedhæftet billede billedet i sit eget vindue i stedet. doda har en lytter, der åbner eksterne links selv (fordi iOS ikke gør det i en app på hjemmeskærmen), og den nåede først: vinduet var åbnet, før billedruden kunne nå at sige fra. Ruden blev faktisk tegnet — den lå bare bagved. Nu lytter den før alt andet. |
| 90 | **Klik på et billede — og kopiér det.** Billeder i Sagu-noter, i projektbeskrivelser, i notekort og i vedhæftninger åbner nu stort, med **Copy image**, **Open** og en lukkeknap. Esc og swipe lukker også. Det er **Sagus** billedrude, ned til ikonerne: det er ofte Sagus billeder, du klikker på, og en note vist inde i doda skal ikke føles som et fremmed sted. Billedet lægges på udklipsholderen som et rigtigt billede — ikke som en adresse, der kræver at modtageren er logget ind. |
| 89 | **Notifikationer virker på iPhone — og teksten om dem passer igen.** Prøven fra v88 gav svaret: en push *med* tekst kommer frem, en **tom** gør aldrig. iOS vækker aldrig dodas baggrundsproces, og det var derfor syv rettelser i træk slog fejl — de forsøgte alle at gøre et led hurtigere, som ikke findes på telefonen. Beskrivelsen under Notifications sagde stadig »the push itself is empty«, og det har ikke passet siden v87: teksten rejser med, **krypteret med nøgler der kun findes på din enhed**. Løftet er det samme, vejen er en anden. Måleknappen er væk igen — den havde svaret på sit spørgsmål — og en tilmelding uden nøgler er nu markeret, for den kan ikke nå en iPhone. |
| 88 | **En relativ adresse i notifikationen — rettet.** v87 lagde teksten med i pushen, men sendte ikonets adresse relativt (`./icon-192.png`). Nyttelasten læses af *systemet*, ikke af en side, så der er ingenting at opløse den imod — og en streng læser kasserer så hele notifikationen. Det gav præcis det, der skete: Apple kvitterede, intet blev vist, og doda blev aldrig vækket. Alle adresser er nu absolutte. **Og en ny prøve:** »Send a test (empty)« sender den gamle form uden tekst, så det kan afgøres, om pushen slet ikke når frem — eller om det er teksten, der er galt. |
| 87 | **Notifikationen kommer nu med i selve pushen — krypteret.** Seks forklaringer på den manglende push på iPhone er prøvet af og faldet. Tilbage stod ét led: iOS skal vække en baggrundsproces og lade den køre kode, før noget kan vises. Nu bærer pushen selv teksten, så **systemet viser den direkte** uden at vække noget. **Og nej, Apple kan ikke læse den:** teksten er krypteret med nøgler, der kun findes på din telefon. Løftet om, at push-tjenesten aldrig får at vide, hvad dine opgaver hedder, gælder uændret. Kan din browser ikke den nye form, virker den gamle vej som før — bare hurtigere, fordi der ikke skal hentes noget. **Og der er nu en måling:** under Notifications står det, om doda overhovedet er blevet vækket af en push. |
| 86 | **Pushen beder nu om at blive leveret straks — og to fejl, der gjorde fejlsøgningen umulig, er væk.** doda sendte ikke `Urgency: high`, og uden den header er hastegraden »normal«: push-tjenesten *må* udsætte leveringen for at spare strøm. En påmindelse, der kommer en time senere, er ikke en påmindelse. **Vigtigere:** »Send a test« sendte til *alle* enheder, hvis den, du sad med, ikke var tilmeldt — så svarede den »kom igennem« om nogen andres telefon. Og »slå fra på denne enhed« kunne afmelde **alle** enheder, hvis telefonen ikke kunne finde sit eget abonnement. Begge dele fik prøverne til at lyve. Kvitteringen viser nu også Apples `apns-id`. |
| 85 | **Ringen i den åbne opgave afslutter den — og lukker.** Har du en opgave åben, var afkrydsningsringen en indstilling som alle de andre: den farvede sig selv og ventede på **Save**. Men en ring er ikke noget, man overvejer — det er den ene handling, hele listen handler om, og i listen afslutter den samme ring med det samme. Nu gør den det også herinde: ét tryk, opgaven er klaret, ruden lukker, og du står i listen. Med **Undo** i kvitteringen som altid. Har du skrevet noget i ruden uden at gemme, kommer det med — det ville være mærkeligt at kvittere for en opgave og se sin egen tekst forsvinde. **Chips er urørte:** de venter stadig på Save, så et fejlklik på en dato ikke gemmer noget bag om dig. |
| 84 | **Opdaterings-knappen kan ikke længere trædes over tæerne — og redningsvejen fik de regler, hovedvejen havde.** Runens `update:` har en gren, der kun bruges, når doda opgraderes fra en udgave uden selv-hentning — altså præcis den ene gang, det gælder. Den gren pakkede stadig ud i `/tmp` og **slettede** `app/`, før den nye kom på plads, så der var et øjeblik helt uden app — og startup-redningen dækkede ikke den vej. Nu flyttes den gamle app væk i stedet, ved siden af den nye, hvor et skift af navn ikke kan afbrydes. **Og:** trykker man på knappen to gange, siger den nu fra i stedet for at lade to kørsler rode i de samme filer. Fundet af Sagu, som mistede ti timer på det. |
...
| 78 | **Se og ryd de enkelte push-tilmeldinger.** »6 devices« sagde intet om, hvilke der stadig virkede — og de hober sig op, fordi hvert »slå til« giver en ny, mens de gamle bliver liggende. Apple svarer `201` på dem alligevel, så prøven sagde *kom igennem* seks gange uden at fortælle, om **din** telefons nåede frem. Nu står de på en liste med tjeneste, hvornår de blev oprettet, **hvornår de sidst kvitterede** og fejl i træk — med **denne enhed** markeret og en **Fjern**-knap ved hver. Og »Send a test« rammer nu kun den enhed, du sidder med. Endepunktet forlader aldrig serveren: listen viser værtsnavnet og en hash. |
