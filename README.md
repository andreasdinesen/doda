...
| 30 | **Forslag mens du skriver et navn.** Skriver du `/som`, `@som` eller `#h` midt i en linje, viser kommandobaren de projekter eller kontekster, der matcher — det, der *begynder* med det skrevne, står øverst. **Tab** sætter navnet ind, og navne med mellemrum kommer automatisk i anførselstegn. Forslagene står under oprettelsen, så **Enter fanger stadig**, uændret. `andreas@omlidt.dk` udløser ingen liste — forslagene følger nøjagtig samme regler som parseren. **Dertil:** søgningen bruger nu den tolkede titel, så resultaterne ikke forsvinder, i det øjeblik du tilføjer `/projekt` eller `!i morgen` til linjen. |
