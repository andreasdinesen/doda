 ...
 *    omdoebninger ligger app/ under `.doda-gammel` - og startup-kommandoen
...
const NY = path.join(ROD, '.doda-ny');
const GAMMEL = path.join(ROD, '.doda-gammel');
 ...
 * »82.1«, skal man vide, at doda ikke gjorde, hvad der stod.
 */
 ...
 * install-script (eller en aeldre doda) - og saa staar tallet i index.html,
...
function hentSvar(url, forsoeg, ved) {
  return new Promise((ok, nej) => {
    const req = https.get(url, {
      headers: { 'user-agent': 'doda-opdatering', accept: '*/*' },
      timeout: TIMEOUT_MS,
    }, (res) => {
...
/**
 * Henter og pakker `vN` ud i .doda-ny/ og returnerer stien til app-mappen.
 *
...
/**
 * Er det her en hel doda - og er det den, vi bad om?
 *
...
function tjekTrae(mappe, version) {
  for (const kraevet of ['server.js', 'public/index.html', 'public/app.js', 'shared/parse.js']) {
 ...
 *
 * Doer processen mellem dem, ligger den gamle app under `.doda-gammel`, og
...
async function opdater(env = process.env) {
   ...
   * genstart ikke mere. Vejen tilbage er panelets »Opdater doda«, som falder
   * tilbage til runens startsnor. Det skal staa i loggen, FOER det sker. */
  if (maal < 82) {
    advar(`v${maal} er fra foer doda kunne hente sin egen kode.`);
    advar('En genstart opdaterer ikke derfra - brug »Opdater doda« i panelet for at komme videre.');
  }
