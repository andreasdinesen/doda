...
test('strukturen er, hvad en scanner leder efter', () => {
  const { modules: m, size: n } = Q.lavQr('otpauth://totp/doda:test?secret=ABCDEFGHIJKLMNOP');
...
test('koden kan læses tilbage - en rigtig otpauth-adresse', () => {
  const uri = 'otpauth://totp/doda:andreas?secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ'
    + '&issuer=doda&algorithm=SHA1&digits=6&period=30';
...
test('begge kopier af format-informationen siger det samme', () => {
  for (const tekst of ['a', 'otpauth://totp/doda:andreas?secret=GEZDGNBVGY3TQOJQ', 'x'.repeat(120)]) {
...
test('version 7 og opefter bærer versionsinformation', () => {
  ...
  const stor = Q.lavQr('otpauth://totp/doda:andreas?secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ'
    + '&issuer=doda&algorithm=SHA1&digits=6&period=30');
...
test('macOS kan læse koden — den prøve, min egen afkoder ikke kan give', { skip: !harSwift && 'swift findes ikke her' }, () => {
  ...
  try {
    const proever = [
      'a',
      'otpauth://totp/doda:a?secret=ABCDEFGH',
      'otpauth://totp/doda:andreas?secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ&issuer=doda',
      'otpauth://totp/doda:andreas?secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ'
        + '&issuer=doda&algorithm=SHA1&digits=6&period=30',
      'x'.repeat(150),
    ];
...
test('format-informationen melder niveau M', () => {
  const { modules: g } = Q.lavQr('otpauth://totp/doda:a?secret=ABCDEFGH');
  let fmt = 0;
  for (let i = 0; i <= 5; i++) fmt |= g[8][i] << (14 - i);
  fmt |= g[8][7] << 8;
  fmt |= g[8][8] << 7;
